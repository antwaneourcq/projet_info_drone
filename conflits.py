...
def coef_directeur(A, B):
    if A.x == B.x:
        return 0
    return (B.y - A.y) / (B.x - A.x)

def origine(A,B) :
    return A.y - coef_directeur(A,B)*A.x
...
def point_intersection(A, B, C, D):  # a l'instant t AB et CD
    ...
    a1 = coef_directeur(A, B)
    b1 = origine(A, B)
    a2 = coef_directeur(C, D)
    b2 = origine(C, D)
    print('1 :', a1 ,'x + ', b1 ,'2 :  ',a2, 'x + ',b2)
    #Traiter avant le cas des droites identiques i.e. a1 == a2 and b1 == b2 (nombre de points d'intersection infini)
    if a1 != a2:
        Ix = (b1 - b2)/(a2 - a1)
        Iy = Ix * a1 + b1
        I = geo.Point(Ix, Iy, 0)
...
def appartenance_segment(point, A,B): ###s'assurer que A, B et point sont de classe point avec attribut x, y et non long et lat
    if coef_directeur(A,B)*point.x + origine(A,B) == point.y :
        if A.x < B.x :
            if A.y<B.y:
...
def interception (A, B, C, D, I) :
    #I correspond à point_intersection(A,B,C,D)
    if I:
